...
class ProfessionalController {
    ...
    public ResponseEntity<List<ProfessionalResponseDTO>> getAllProfessionals(
        ...
        List<ProfessionalResponseDTO> professionals = professionalService.findProfessionals(name, specialtyId);
    ...
    public ResponseEntity<ProfessionalResponseDTO> createProfessional(@Valid @RequestBody ProfessionalRequestDTO requestDTO) {
        ProfessionalResponseDTO newProfessional = professionalService.createProfessional(requestDTO);
        return ResponseEntity.status(201).body(newProfessional);
    }

    @PatchMapping("/{id}")
    @PreAuthorize("hasAnyRole('ROLE_ADMIN')")
    public ResponseEntity<ProfessionalResponseDTO> patchProfessional(
            @PathVariable Long id,
            @RequestBody Map<String, Object> updates) {

        ProfessionalResponseDTO updatedProfessional = professionalService.patchProfessional(id, updates);
